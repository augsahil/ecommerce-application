...
const AddProduct = () => {
  ...
  const validationSchema = yup.object({
    ...
    category: yup
      ...
      .oneOf(Object.keys(Categies), 'Choose Valid Category'),
  ...
  return (
    <>
      {images && images.length > 0 ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2  lg:grid-cols-4 mx-auto gap-x-3 gap-y-2">
            {images.map((cur, i) => {
              return (
                <div
                  key={i}
                  className="w-full relative h-[200px] p-2 rounded-sm"
                >
                  <img
        ...
        <div
          {...getRootProps()}
          className="border w-full min-h-44 flex justify-center items-center border-dashed border-blue-500 bg-white"
        >
          <input {...getInputProps()} />
          {isDragActive ? (
